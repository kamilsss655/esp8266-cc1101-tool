...
static void createMessage(int id, int state) {
	...
	if(state == 1) {
		json_append_member(selectremote->message, "on", json_mknumber(1, 0));
	} else {
		json_append_member(selectremote->message, "off", json_mknumber(1, 0));
	}
}
...
struct protocol_t *selectremote = NULL;
