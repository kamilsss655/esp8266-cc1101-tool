...
static void createMessage(int systemcode, int unitcode, int state) {
	...
	if(state == 0) {
		json_append_member(logilink_switch->message, "on", json_mknumber(1, 0));
	} else {
		json_append_member(logilink_switch->message, "off", json_mknumber(1, 0));
	}
}
...
struct protocol_t *logilink_switch = NULL;
