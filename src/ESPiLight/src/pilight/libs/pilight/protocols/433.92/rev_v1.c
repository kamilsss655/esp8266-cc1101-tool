...
static void createMessage(char *id, int unit, int state) {
	...
	if(state == 1)
		json_append_member(rev1_switch->message, "off", json_mknumber(1, 0));
	else
		json_append_member(rev1_switch->message, "on", json_mknumber(1, 0));
}
...
struct protocol_t *rev1_switch = NULL;
