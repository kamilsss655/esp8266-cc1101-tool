...
static void createMessage(int systemcode, int unitcode, int state) {
	...
	if(state == 1) {
		json_append_member(elro_400_switch->message, "on", json_mknumber(1, 0));
	} else {
		json_append_member(elro_400_switch->message, "off", json_mknumber(1, 0));
	}
}
...
struct protocol_t *elro_400_switch = NULL;
